...
def learn():
    ...
    for i in range(1, num_iterations + 1):
        ...
        # model_new = train(copy.deepcopy(model), examples, num_epochs=10)
        model = arena(copy.deepcopy(model), copy.deepcopy(model_new), device=device, num_games=40, win_threshold=0.55)
        torch.save(model.state_dict(), "../../models/saved/dqn_cnn_v2_7.pth")
        ...
        # result: model accepted 0.325/0.025/0.65 (moderate reliability) => (lr: 0.00001, cpuct: 4.0, dirichlet = 1.0)
        # check again: effect of alpha in dirichlet distribution
        # hyperparameter check: first try
